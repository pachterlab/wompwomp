...
# font_import(prompt=FALSE)  # Run once to import fonts
# loadfonts()
...
# Define your process_and_plot_alluvial function here
process_and_plot_alluvial <- function(input_csv, column1 = NULL, column2 = NULL, show_group_2_box_labels_in_ascending = FALSE, output_path = NULL) {
    adata_obs <- read.csv(input_csv)
    
    # Infer column1 and column2 if not specified
    if (is.null(column1) || is.null(column2)) {
        candidate_columns <- setdiff(colnames(adata_obs), c("experiment_accession", "id", "identifier"))  # Exclude ID-like columns
        if (length(candidate_columns) < 2) {
            stop("Unable to infer column1 and column2: insufficient valid columns in adata_obs.")
        }
        if (is.null(column1)) column1 <- candidate_columns[1]
        if (is.null(column2)) column2 <- candidate_columns[2]
    }
    ...
    alluvial_plot <- plot_alluvial(clus_df_gather, group1_name = "tissue", group2_name = "leiden", group1_name_mapping = "tissue_name", group2_name_mapping = "leiden_mapping", color_boxes = TRUE, color_bands = TRUE, match_colors = TRUE, alluvial_alpha = 0.5, include_labels_in_boxes = TRUE, include_axis_titles = TRUE, show_group_2_box_labels_in_ascending = show_group_2_box_labels_in_ascending, output_path = output_path)
...
main <- function() {
    ...
    option_list <- list(
        make_option("--input_csv", type = "character", help = "Path to the input CSV"),
        make_option("--column1", type = "character", help = "Name of column1"),
        make_option("--column2", type = "character", help = "Name of column2"),
        make_option("--output_path", type = "character", default="./alluvial_plot.png", help = "Output base path"),
        make_option("--show_group_2_box_labels_in_ascending", action = "store_true", default=FALSE, help = "Flag to show group 2 box labels in ascending order")
    )
    ...
    # Call the function
    alluvial_plot <- process_and_plot_alluvial(
        input_csv = opt$input_csv,
        column1 = opt$column1,
        column2 = opt$column2,
        output_path = opt$output_path,
        show_group_2_box_labels_in_ascending = opt$show_group_2_box_labels_in_ascending
    )
}
