...
output:
    BiocStyle::html_document:
        ...
        toc_depth: 2
vignette: >
  %\VignetteIndexEntry{1. Quick Start: An Introduction to wompwomp}
...
#     theme_minimal()
# p1
p1 <- plot_alluvial(clus_df_gather, graphing_columns = c("tissue", "cluster"), column_weights = "value", sorting_algorithm = "None", color_boxes = FALSE, color_bands = TRUE, match_order = "None", output_plot_path = file.path(output_dir, "intro_unsorted.pdf"), text_size = 15, axis_text_size = 25, axis_text_vjust = 0)
p1
...
With color
```{r}
p1_5 <- plot_alluvial(clus_df_gather, graphing_columns = c("tissue", "cluster"), column_weights = "value", sorting_algorithm = "None", color_boxes = TRUE, match_order = "advanced", resolution = 1, color_bands = TRUE, output_plot_path = file.path(output_dir, "intro_unsorted_matched_colors.pdf"), text_size = 15, axis_text_size = 25, axis_text_vjust = 0)
p1_5
```
...
```{r}
p2 <- plot_alluvial(clus_df_gather, graphing_columns = c("tissue", "cluster"), column_weights = "value", sorting_algorithm = "greedy_WOLF", fixed_column = 1, color_bands = TRUE, match_order = "advanced", resolution = 1, output_plot_path = file.path(output_dir, "intro_WOLF.pdf"), text_size = 15, axis_text_size = 25, axis_text_vjust = 0)
...
```{r}
p3 <- plot_alluvial(clus_df_gather, graphing_columns = c("tissue", "cluster"), column_weights = "value", sorting_algorithm = "greedy_WBLF", color_bands = TRUE, match_order = "advanced", resolution = 1, output_plot_path = file.path(output_dir, "intro_WBLF.pdf"), text_size = 15, axis_text_size = 25, axis_text_vjust = 0) # output_plot_path = "vignette_intro_plot.png"
...
```{r}
clus_df_gather_sorted_nn <- data_sort(clus_df_gather, column1 = "tissue", column2 = "cluster", column_weights = "value", sorting_algorithm = "neighbornet") # output_df_path = "vignette_intro_df_cluster_wblf.csv"

plot_alluvial(clus_df_gather, graphing_columns = c("tissue", "cluster"), column_weights = "value", sorting_algorithm = "neighbornet", color_boxes = FALSE, color_bands = TRUE, match_order = "None", resolution = 2, output_plot_path = file.path(output_dir, "intro_NN_without_color.pdf"), text_size = 15, axis_text_size = 25, axis_text_vjust = 0)
...
p4 <- plot_alluvial(clus_df_gather, graphing_columns = c("tissue", "cluster"), column_weights = "value", sorting_algorithm = "neighbornet", color_bands = TRUE, match_order = "advanced", resolution = 1, output_plot_path = file.path(output_dir, "intro_NN.pdf"), text_size = 15, axis_text_size = 25, axis_text_vjust = 0)
...
my_func <- function(a = "Foo", b = "Bar", c = 42) {
  lowercase_args(c("a", "b"))
  print(list(a = a, b = b, c = c))
  browser()
}

my_func(a = "HELLO", b = "WORLD", c = 99)
