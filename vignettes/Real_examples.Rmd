...
if (!file.exists(kits_file)) {
    stop("KiTS file not found, and is currently not available for download")
}
...
plot_alluvial(df, graphing_columns = graphing_columns, sorting_algorithm = "neighbornet", color_bands=TRUE, output_plot_path = file.path(output_dir, "Segmentation_stratified_KiTS_NN.pdf"), min_text = 0.01, save_height = 24, save_width = 24)
...
```{r}
p <- plot_alluvial(df, graphing_columns = graphing_columns, sorting_algorithm = "None", color_bands=TRUE, verbose=TRUE, output_plot_path=file.path(output_dir, "GoT_unsorted.pdf"), min_text = 0.01, save_height = 24, save_width = 24)
p
```
