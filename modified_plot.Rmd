...
all_df <- read_csv('/workspace/metadata_csvs/alluvial_inputs.csv')

#all_df$tissue = paste(all_df$tissue, "tissue", sep="_")
#all_df$leiden_tissue = paste(all_df$leiden_tissue, "cluster", sep="_")

all_df$matched <- (all_df$tissue_cluster == all_df$leiden) | all_df$count > 5
all_df$swapped <- !all_df$matched & (all_df$tissue == 'HIPPOC' | all_df$tissue == 'HYPOTH')
all_df$leiden <- factor(all_df$leiden)
all_df$tissue_cluster <- factor(all_df$tissue_cluster)
all_df$matched <- ifelse(all_df$matched, '', 
                         ifelse(all_df$swapped, 'Swapped', 'Mismatched'))
#all_df$tissue <- factor(all_df$tissue)
#all_df$leiden_tissue <- factor(all_df$leiden_tissue)


all_df$tissue_num <- factor(as.numeric(factor(all_df$tissue)))
all_df$leiden_num <- factor(as.numeric(factor(all_df$leiden)))
renumbered <- c()
current = 1
for (i in seq_along(all_df$leiden)){
    current_entry <- all_df$leiden[i]
        if (!(current_entry %in% all_df$leiden[0:(i-1)])){
            renumbered <- c(renumbered, current)
            current <- current +1
        } else{
            renumbered <- c(renumbered,renumbered[which(all_df$leiden == current_entry)[1]])
        }
}
#all_df$leiden_num <- factor(renumbered)

final_colors <- color_opts
all_df$tissue = toupper(all_df$tissue)
all_df$leiden_tissue = toupper(all_df$leiden_tissue)
plot_alluvial(all_df, group1_name = 'tissue_num', group2_name = 'leiden_num', ditto_colors = final_colors, color_boxes = TRUE, color_bands = TRUE, save = '/workspace/output/figure_1c.png')
```
