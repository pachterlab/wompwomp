...
```{r}
clus_df_gather_sorted <- sort_to_uncross(df, cols = c("tissue", "cluster"), method = "greedy", fixed_column = "tissue", options = list(default_sorting = "fixed"))
wompwomp_objective_greedy_wolf <- compute_crossing_objective(clus_df_gather_sorted, cols = c("col1_int", "col2_int"))$output_objective
print(wompwomp_objective_greedy_wolf)
```

Plot the alluvial plot with sorting and color matching
```{r}
p1 <- plot_alluvial(clus_df_gather_sorted, column1 = "tissue", column2 = "cluster", column_weights = "value", sorting_algorithm = "greedy", fixed_column = "tissue", default_sorting = "fixed", color_bands = TRUE)
p1
```
...
for (i in 1:nrow(perms)) {
    ...
    clus_permuted_objective <- compute_crossing_objective(clus_permuted, cols = c("col1_int", "col2_int"))$output_objective
...
```{r}
clus_df_gather_sorted <- sort_to_uncross(df, cols = c("tissue", "cluster"), method = "greedy", options = sort_to_uncross_options(random_initializations = 10))
wompwomp_objective_greedy_wblf <- compute_crossing_objective(clus_df_gather_sorted, cols = c("col1_int", "col2_int"))$output_objective
print(wompwomp_objective_greedy_wblf)
```

```{r}
p2 <- plot_alluvial(clus_df_gather_sorted, column1 = "tissue", column2 = "cluster", column_weights = "value", sorting_algorithm = "greedy", color_bands = TRUE)
p2
```
...
for (i in 1:nrow(left_perms)) {
    ...
    for (j in 1:nrow(right_perms)) {
        ...
        # Compute objective
        obj <- compute_crossing_objective(clus_permuted, cols = c("col1_int", "col2_int"))$output_objective
        objectives[i, j] <- obj
...
```{r}
clus_df_gather_sorted_neighbornet <- sort_to_uncross(df, cols = c("tissue", "cluster"), method = "neighbornet")
neighbornet_objective_minimum <- compute_crossing_objective(clus_df_gather_sorted_neighbornet, cols = c("col1_int", "col2_int"))$output_objective
