...
Optional:
  ...
  --set_seed                Random seed for the random_initializations parameter (only applies when sorting_algorithm == 'greedy_WOLF' or sorting_algorithm == 'greedy_WBLF'), random initialization/shuffling of blocks (only applies when default_sorting == 'random' or sorting_algorithm == 'random'), TSP solver for block order or optimizing column order (only applies when sorting_algorithm == 'tsp' or column_sorting_algorithm == 'tsp'), and louvain/leiden clustering (only applies when match_order == 'advanced'). Depracated (recommended to set seed outside of fucntion call).
