...
find_group2_colors <- function(clus_df_gather, group1_name, group2_name, 
                               weight='w', ditto_colors = paletteer::paletteer_d("ggthemes::Classic_Cyclic")) {
    clus_df_filtered <- clus_df_gather[, c(group1_name, group2_name, weight)]
    ...
    matching <- igraph::max_bipartite_match(g, weights = clus_df_filtered[[weight]])
...
```{r}
plot_alluvial <- function(clus_df_gather, group1_name = "a", group2_name = "b", 
                               weight='w', group1_name_mapping = "a", group2_name_mapping = "b", color_boxes = TRUE, color_bands = FALSE, alluvial_alpha = 0.5, match_colors = TRUE, save = FALSE, include_labels_in_boxes = FALSE, include_axis_titles = FALSE, show_group_2_box_labels_in_ascending = FALSE, axis_text_size=4, ditto_colors = paletteer::paletteer_d("ggthemes::Classic_Cyclic")) {
    ...
    p <- ggplot(data = clus_df_gather, aes(axis1 = !!sym(group1_name), axis2 = !!sym(group2_name), y = weight))  # uncommented as of Jan 2025
    ...
    if (include_axis_titles) {
        p <- p +
            annotate("text", x = 1, y = max(clus_df_gather[[weight]]) + 510, label = group1_name, size = 5, hjust = 0.5) +
            annotate("text", x = 2, y = max(clus_df_gather[[weight]]) + 510, label = group2_name, size = 5, hjust = 0.5)
    } 
...
plot_alluvial(all_df, group1_name = 'a', group2_name = 'b', color_boxes = TRUE, color_bands = TRUE)
